				...
				)
{
	...
	if ( is.null(Edes) ){			
		Edes <- mml_3pl_nonzero_entries( E=as.vector(E) , dimE=dim(E) )$E_design
	}											
    ...
    for (i in 1:nitems){ 
		resp.ind.list[[i]] <- which( resp.ind[,i] == 1)  
	}
    ...
    equal.categ <- if( sd.maxscore.resp > 1E-5 ){ FALSE } else { TRUE  }
	...
	dimFdes <- dim(Fdes) 
	res <- mml3_calc_Fdes( XDES=as.vector(Fdes) , dimXdes=dimFdes )
