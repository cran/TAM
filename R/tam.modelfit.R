...
# cat("p values") ; zz1 <- Sys.time(); print(zz1-zz0) ; zz0 <- zz1					
	#**** fit statistic
	stat.MADaQ3 <- data.frame( "MADaQ3" = mean( abs(dfr$aQ3) ) )	
...
# cat("calc counts V2") ; zz1 <- Sys.time(); print(zz1-zz0) ; zz0 <- zz1						
	...
	eps <- 1e-10
	chi2.stat$chi2 <- rowSums( ( obs_counts - exp_counts )^2 / ( exp_counts + eps ) )
	chi2.stat$p <- 1- pchisq( chi2.stat$chi2 , df=chi2.stat$df )
	chi2.stat$p.holm <- p.adjust( chi2.stat$p , method="holm")
	#*****
    ...
    residcov <- 100*mean( abs( res2$cov_ij - res2e$cov_ij ) )
